...
- **NEXT (honest):** (1) confirm what the dispatch-mode create-path constructs (the new ret-x0 object) and whether it is a ScriptContext/app-shell/GuiObject; (2) re-derive the post-DM content path (UniversalApp / GuiObject) against the corrected base. Prior "migration-gate" closures were built on the wrong +8 vptr and need re-derivation.
