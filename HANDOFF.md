...
- **Forward note (new, worth checking before re-blamming live-DM on a next NULL fault):** an UNRESOLVED bind_glob_dat data import leaves its GOT slot 0 -> NULL deref; check (bound, unresolved) return where the crash region's only imported data is one of the 78.
