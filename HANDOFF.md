...
NEXT GATE (now exposed — pre-existing concurrency race, not a regression): with real rendering active, the combined (render-thread guest jit_run + boot-ladder guest jit_run) tears shared guest-global state during nativeGameGlobalInit and aborts `*** stack smashing detected ***`. All guest contexts (boot main-stack, render 1MiB rstack from SH108, worker 16MiB from spawn_pthread) ARE stack-isolated, and SH100 guards the block-cache EVICTION (ACTIVE_TOP_LEVEL_RUNS), so the smash is the remaining SH55/64-class "two threads mutate the do-init walker's seeded globals concurrently" race (jit.rs:2008). It only fires NOW because renderinit finally completes + renders. Fix direction: serialize top-level guest execution while the ladder drives the do-init (the render thread's frame-fn work can wait), or scope the seeded globals so concurrent guest mutation can't tear them. Anything that works alone, works against the render thread.
