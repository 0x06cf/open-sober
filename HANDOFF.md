...
## SH220 (Sep 16, 2026, hermes-worker): R1 content-path reachability = MEASURED 3-way negative (answers the operator's SH219b-delegated "log the engine's first rbxasset:// request to name the R1 CoreScript file" = there is no request). Commit <SH220COMMIT>. Doc docs/frontier-sh220-r1-reachability.md, repro runs/capture_sh220_r1_reachability.sh. Workspace green (arm64jit 388/0 + elfjit example 68/0 + all crates). No production code change (pure measurement of the already-shipped JIT_ASSET_TRACE diagnostic).
