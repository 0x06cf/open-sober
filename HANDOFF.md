...
## SH94 (Sep 13, 2026, hermes-worker): killed the post-SH93 6.6M-iteration registrar spin — don't clobber the INSERT-entry x19 walk iterator. Recon deleg_3f812009: the post-barrier do-init body runs cleanly to its ret (file 0x2206e9c, no 2nd CEvent); the spin was pre-barrier, in the pb/Otel registrar loop (file 0x29b37e0: `mov x1,x19; bl 29f3e70; ldr x8,[x19,#16]!; cbnz`), where x19 is the CALLEE-SAVED walk ITERATOR/KEY, NOT the map. SH92 had been substituting x19 too; since x19 is callee-saved, the corrupted value returned to the caller made `[x19+16]` read the substitute's +0x10=SPAN_HASH (nonzero) forever -> `cbnz` never terminated -> 6.6M-iteration spin (`SH92 substituted ... reg=x19`). Fix (jit.rs): at INSERT entry substitute x0 unconditionally (when non-family); substitute x19 ONLY when x0 is also non-family (the real .data-table crash case where x19 held the sibling span map). VERIFIED: 6.6M reg=x19 flood GONE (subx0 is a sane 49); registrar loop terminates; ladder hits a NEW shallower crash (guestpc 0x1029b37ec registrar back-edge / 0x1029f4034 INSERT-tail, fault==rip==heap, run-variable 134/139) — the loop now runs its real body and a deeper fn-ptr dispatch faults. Workspace 516/0. Product unregressed (124, persist byte-exact, 0 crash). Doc frontier-sh94-x19-iterator-spin.md. NEXT: root-cause the 0x1029b37ec/0x1029f4034 fault==rip==heap (a freshly-allocated per-node or registration callback fn-ptr slot holding host garbage -> seed to a real in-image fn / host-call, SH81/86-style) -> then gameGlobalInit RETURNS -> rung 2 nativeUpdateAdapterInit (0x10221c3ec) -> type-4 [0x106829ea8] -> NativeHelper callbacks.
