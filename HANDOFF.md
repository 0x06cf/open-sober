...
## SH182 (Sep 15, 2026, hermes-worker): HOST-DRIVE the manufactured genuine-vptr DM through its REAL app-shell ctor — Route-B manufactured-DM line gets its FIRST headless DM-vtable execution. Workspace green (arm64jit 374/0). Commit 699feae. Doc docs/frontier-sh182-dm-ctor-driver.md, repro runs/capture_sh182_dm_ctor_driver.sh.
- **CONE (deleg_661626bb, READ-ONLY, 2 authoritative):** (1) full decode of app-shell ctor 0x1057d6ef4 ([V+0x30] of genuine primary DM vtable 0x1067162f0, SH181): reads ONLY the stack-canary global file 0x67d16f0 (guest 0x1067d16f0, VALUE 1 in bare boot -> `ldr x8,[x21]`@0x57d6f10 SEGV; any stable value auto-passes) + DM+0x38c (4 readable bytes, 0 fine). The SH181-flagged `[x1,#8]` deref is a std::string GATE, NOT a code-pointer: [x1+8]==0 -> PATH A clean no-op+ret; == "ServerRestartScheduled" -> PATH B real init (component ctor 0x2bc4f64, AppBridgeV2Init 0x238e0bc, post-client-settings 0x2286ed0, placeVersion vector append 0x22b737c). Ctor does NOT build SceneGraph/render-world/DM sub-objects. PATH-B SSO layout vs equality fn 0x2152f30 (+0/+8/+16 LONG form; w9=0x2c=44 vs 22-char length UNRESOLVED) -> do NOT lock unverified bytes; shipped PATH A. (2) host API = run_guest_callback(fn, args[8], tpidr) (jit.rs:3802, args[1]=x1 FULLY host-controlled); same-thread NESTED run_guest_callback is the sanctioned type4_frame pattern (IN_JIT_RUN keeps cache warm, no SH55/64; SH151 = GL-reentrancy only, ctor is pure .text).
- **CODE (arm64jit jit.rs, default-inert, +1 hermetic test 373->374):** `routeb_dm_ctor_driver_guard` (env JIT_ROUTEB_DM_CTOR_DRIVER, scoped [0x1023efe2c,0x1023eff20], OnceLock-idempotent) seeds canary 0x1067d16f0 to a stable word + nested-run_guest_callback(0x1057d6ef4, x0=routeb_manufactured_dm, x1=routeb_dm_ctor_arg_empty()) -> PATH A survival no-op. `routeb_dm_ctor_arg_empty` = leaked zeroed 0x28 buffer (+8..+0x20 EMPTY SSO std::string). Test: env-off immediate / wrong-pc region-gate / descriptor zeroed.
- **EMPIRICAL (real libroblox.so, llvmpipe):** `[region-watch] entered region 0x1057d6ef4-0x1057d7100 at guest pc=0x1057d6ef4` **(SH181 was 0 — DM vtable now DISPATCHES headlessly)** + `DROVE ok ret x0=<dm> vt=0x1067162f0 entered AND returned through real code` + canary seeded. Ladder clean, EXIT 124, 0 SIGSEGV/SIGABRT/stack-smash. Default env-off unregressed (SH55/64 run-variable flake confirmed pre-existing by 2nd bare run EXIT 124; SH182 fires 0x env-off).
- **HONEST:** first headless execution of the manufactured genuine-vptr DM through its real relocated app-shell ctor — the missing half of SH181. Still PATH A (empty-string no-op): ctor runs real code but does NOT build DM sub-objects. NEXT on this line = PATH B (decode the exact "ServerRestartScheduled" std::string layout vs equality fn 0x2152f30, then seed it to run the real init body); real self-constructed GuiObjects still behind live-DM session = migration gate. Manufacture lever now live-correct (not just latent).
