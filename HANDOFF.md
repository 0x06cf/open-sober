...
## SH310 (Sep 18, 2026, hermes-worker): CONFIRM the fabricated-'Home'-jstring ABI on SendAppEventOnAppReady — the step-2 real-jstring open question (STATUS candidate (b)) is CLOSED by measurement, not static premise. Fresh objdump of SendAppEventOnAppReady body (0x102bb463c) + region-watch decisive observable on real libroblox.so with the SH307-forward env. DISASM pins (authoritative, guest==file+0x100000000): ABI `0x2bb4660 mov x19,x5` (event jstring = arg5, as the step-2 correction predicted; SH155's x2 was wrong); 4x jstring->RBX-string `bl 0x21e1fec` (0x2bb4684/0x2bb4694/0x2bb46a4/0x2bb46b4) convert x2->[sp+0x48]/x3->[sp+0x30]/x4->[sp+0x18]/x5->[sp] — the [sp] slot is discriminated; SSO length decode then len==4 `cmp w9,w10` w10=0x656d6f48 "Home" LE (0x2bb46f8/0x2bb4704) -> `b.eq 0x2bb47c4` (movz w19,#4); version gate 0x2bb47d8 (reads [0x10683d350], low>=6 && byte1>=5 else skip build); if OK -> `bl operator_new(0x58)` app-event (0x2bb48b0), event vtable 0x635e068+0x68, strings copied, `str w19,[x20,#80]` (0x2bb4948), then **`bl 0x2baeeec` pipe @0x2bb4958 -> do-init 0x2206c40**. MEASURED (runs/sh310b/310c-pipe-*.txt): the discriminator+version-gate are MID-BLOCK (SH301: direct b.targets aren't block entries -> my first [0x2bb47bc..) band was 0 hits = OBSERVATION artifact, not non-execution), but the pipe `bl 2baeeec` @0x2bb4958 IS a block entry reached ONLY past discriminator+gate+0x58 alloc — and with --v2boot-skip-appstart it's this rung's ONLY caller, so a hit is UNambiguous. Clean run (EXIT 124, appev=1): pipe 0x102baeeec FIRED + do-init entered + ran the ENTIRE app-shell ctor (0x102206c40 -> 0x102207b50 -> 0x102208e88 past FMOD tail). Runs 1/3 (EXIT 134) also fired pipe+do-init then aborted in do-init (run-variable live-object wall, SH248b 2-band watch perturbation). VERDICT: **candidate (b) CLOSED — the fabricated 'Home' jstring in x5 genuinely resolves (helper 0x21e1fec -> SSO "Home" -> 'Home' w19=4 -> version gate passes -> 0x58 app-event built + dispatched via the pipe -> do-init -> app-shell ctor)**. The observed `w19-event=0x0` across 46 prior logs was the SH308 artifact (x19 callee-saved; post-jit_run read reflects restored value). Step-2's "a fabricated jstring DOES resolve" is now measured + byte-pinned; SendAppEventOnAppReady's 'Home' event genuinely reaches the app-event level. HONEST: no DM (DM-root [0x106a68818]=0, MH_* false); Route-B live-DM structural gate UNCHANGED; SH174 capture-latch single forward hook; anchors the ABI + build+dispatch, NOT a new fixed forward distance (do-init deep construction still hits the live-object wall). +hermetic sh310 (real-image guard, 7 byte-pins: 0x2bb4660=0xaa0503f3/x5, 0x2bb46b4=0x97d8b64e/jstr-helper, 0x2bb4704=0x6b0a013f/Home-cmp, 0x2bb47c4=0x52800093/w19=4, 0x2bb47d8=0x12001c08/version-gate, 0x2bb4948=0xb9005288/ev+80, 0x2bb4958=0x97ffe965/pipe). Verify: `cargo test -p arm64jit --example elfjit -- sh310` 1 passed; elfjit examples 135/0 (was 134); workspace green pending. elfjit.rs held under the 1MB pre-commit hook (97 B under; condensed SH81/177/224/245/264/285/295 + render-doc prose, all addresses/constants preserved — no behavior touched). Doc docs/frontier-sh310.md; repro runs/capture_sh310b_pipe.sh + capture_sh310c_pipe_clean.sh. Single-agent.
## SH309 (Sep 18, 2026, hermes-worker): MEASURE the SendAppEvent spine CLOSURE (STATUS candidate (a)) + anchor the StartAppWithParams ABI and the provably-unreached session-ctor gate prologues. With the SH307-forward env the `SendAppEventOnAppReady` pipe runs nativeAppBridgeV2StartAppWithParams (0x10258b144) through its own `bl 2baeeec` -> do-init (0x10258b2dc=0x94188f04, vt[+296] dispatch 0x10258b630=0xf9409508, clean ret 0x10258b698=0xd65f03c0), but the downstream session-ctor gates stay **0 region-hits**: post-do-init 0x1023eff4c (=0xd10603ff), governor 0x102e9fa84 (=0xa9ba7bfd), EC-world 0x102e24598, ScriptContext loader 0x101f1d8ac (=0x90022ca8). Every gate is a REAL code entry (authoritative guest-0x100000000==file-offset byte read, 4-aligned, in-window), so the closure is meaningful, not a bad-address artifact: the pipe self-terminates at StartApp clean without reaching any DM/EC/Lua construction. +hermetic sh309 (spine bl/ldr/ret + post-do-init/governor/ScriptContext prologues; examples 134/0 was 133). elfjit.rs 1,048,570 B = 6 B under the 1MB hook (funded by condensing SH306/307/308/126/272 comment prose — facts preserved). HONEST: does NOT manufacture a DM (DM-root 0, MH_* false); Route-B live-DM structural gate UNCHANGED; spine reach is run-variable (multi-band region-watch perturbs it, SH248b class) so this cycle anchors ABI+unreached gates, not a fixed forward distance; SH174 capture-latch stays the single forward hook. Verify: `cargo test -p arm64jit --example elfjit -- sh309` 1 passed; elfjit examples 134/0; workspace green (405 lib). Doc docs/frontier-sh309.md; repro runs/sh309-rw.txt + runs/capture_sh309_startapp_spine.sh. Single-agent.
## SH308 (Sep 18, 2026, hermes-worker): RE-VERIFY the SH307 forward at newest HEAD + MEASURE its reach. SendAppEventOnAppReady (SH307-on) now completes AND its app-bridge pipe drives the do-init app-shell ctor 0x102207b50 through its FMOD/AAudio audio-iterate tail + reaches nativeAppBridgeV2StartAppWithParams — firsts on this path, A/B-contrasted. (1) FORWARD (JIT_ROUTEB_PRELOAD_VALUECELL=1): `SendAppEventOnAppReady returned Ok(0x107273d50)`, 0 SIGSEGV, LADDER_DONE=1, EXIT 124. (2) JIT_REGION_WATCH A/B: BASELINE (SH307 off) = 0 do-init/ctor/FMOD/StartApp hits (dies guestpc=0x102bb803c); FORWARD = do-init 0x102206c40 entered (~26 blocks -> 0x10220703x), app-shell ctor 0x102207b50 runs ~31 blocks to 0x102208eac, its FMOD iterate tail (0x102208ebc `b 0x5fb30b4`) executes audio-init body 0x105fb310c..0x105fb31b4, and StartAppWithParams 0x10258b144 reached (18 hits thru 0x10258b5a0). (3) +hermetic sh308 `sh308_sendappevent_doinit_ctor_audio_tail_reach_pinned` (real-image guards: ctor entry 0x102207b50=0x14000001, ctor tail 0x102208ebc=0x14f6a87e, FMOD iterate 0x105fb30b4=0xd10183ff / empty-check 0x5fb30e0=0x540002a0 / audio-body 0x105fb3174=0xd100c3ff, StartAppWithParams 0x10258b144=0xd103c3ff; in-window + 4-aligned; 1 passed, examples 133/0). HONEST: does NOT manufacture a DM (DM-root [0x106a68818]=0, MH_* all false; post-do-init 0x1023eff4c / ScriptContext loader 0x101f1d8ac / CoreScript stay 0 hits); w19-event=0x0 — 'Home' discriminator never resolves (across 46 logged runs w19 is only 0x0/0x1, never 0x4; Step-2 real-jstring ABI gap open, noted not-solved); Route-B live-DM structural gate UNCHANGED; SH174 capture-latch stays single forward hook. Verify: cargo test --workspace EXIT 0 (586/0 = 405 lib + elfjit examples 133/0 + fsmap + others); cargo build EXIT 0; elfjit.rs 1,048,491 B = +85 B under 1MB hook. Doc docs/frontier-sh308-sendappevent-doinit-ctor-reach.md; repro runs/sh308-rw*.txt + runs/sh307-appev-A/B.txt. Single-agent, default-inert (SH307 stays opt-in).
