...
## SH99d (Sep 13, 2026, hermes-worker): gameGlobalInit probe-vector + dispatch-obj seeds (SH99-99d) — ADVANCED past the SH97/98 gates. SH98 (commit 676108b) CLEARED the `*** stack smashing detected ***` abort: `stat/fstat/lstat` were unshimmed, so host glibc wrote its 144-B x86-64 `struct stat` into the guest's stack-local 128-B bionic struct, overrunning the `__stack_chk_guard` canary; new bionic_stat/fstat/lstat marshal into the bionic aarch64 128-B layout. SH99-99d (6a0728e/4259c4b/cb05631/6d94f2e) seeded the walker's probe vector + dispatch object: fault advanced 0x0->0x10->0x18, BOTH virtual `blr` dispatches execute and the once-guard cached path is exercised. The ladder STILL faults (reported 0x102208504, fault 0x18, RUN-VARIABLE across 0x102208450/0x102208504/0x1021db144 on a worker thread). Workspace **518/0**. Next: gameGlobalInit RETURNS -> rung 2 nativeUpdateAdapterInit (0x10221c3ec) -> type-4 [0x106829ea8] -> NativeHelper. (Historical SH-chain below.)
