...
## SH200 (Sep 16, 2026, hermes-worker): V2StartAppWithParams now COMPLETES — the V2Init/V2Start run-variable "outside image" stop is a SCOPED-SEEDABLE singleton-dispatch family (FALSIFIES SH198's "non-seedable host-pointer class"). Commit 824d8ac. Doc docs/frontier-sh200-v2dispatch-seedable.md, repro runs/capture_sh200_v2dispatch.sh. Workspace green (562/0, +1 hermetic; arm64jit example tests 62/0).
- **FINDING:** the V2Init/V2Start soft-return stop (SH198 pinned to `blr x8` at 0x106251eb4, pc varies 0x9e/0x8b/0xb848c300000100/0xc148...) is NOT a genuine host pointer — it is fn 0x6251e0c (V2Init/Start params accessor) reading singleton objB's vtable slot +0x118 (280), PAST the harness-seeded 0x60 leaf vtable, into host box-alloc bytes that look like x86-mcode (hence the run-variable pc). The SITE is fixed + seedable: materialize the stable singleton object into x0 and NOP the blr.
- **CODE (elfjit.rs, default-inert under JIT_SH115_SINGLETON_PATCH, idempotent):** `routeb_patch_v2_dispatch` patches 4 located sites (fn 0x6251e0c +0x118, 0x62523ac +0x130, 0x6258e88 +0x2f0, 0x6258ffc +0x2f8) — dispatch window from `ldr x8,[x0]` guard (0xf9400008) through the `blr` -> movz/movk x0 = stable object + nops, preserving the trailing `ldr x8,[x19]; str x0,[x8]`/`strb w0,[x8]` store (receives the stable obj, no NULL+0x28). Pure `sh200_v2_dispatch_window` + 1 hermetic test (movz/movk round-trip reconstructs obj; tail nops; 4-slot min).
- **EMPIRICAL (real libroblox.so, EXIT 124, 0 crash):** `V2StartAppWithParams returned Ok(0x0)` (was "stopped: outside image" at SH199 HEAD), re-produced; SH200 fires for all 4 sites; V2Init still soft-returns deeper into a ~600-site uniform serialization family (low-ROI to clear — NOT the Route-B critical path; the do-init->StartLuaAppDM->governor continuation runs clean regardless). Default env-OFF path untouched.
- **STANDING (unchanged):** Route-B live-DM/class-registry world-build = the structural gate. SH200 converts SH198's "non-seedable V2 stop" into a located+seedable, empirically-cleared site and un-blocks V2Start completion — a small real ladder advance on the manufacture/DMCONT line. If V2Init's uniform family is ever worth fully clearing, a data-driven scan of the ~600 `bl 0x6249eb8` sites (identical shape) is the tractable path; but V2Init is not Route-B-critical, do not prioritize over the live-DM gate.
...
- **CODE (arm64jit jit.rs):** `jit_run_inner` keeps a tiny `[u64;16]` ring of the last block-entry pcs (2 stores/iter, unconditional, negligible; 16 is power-of-2 so `& 15` wraps exactly). At the out-of-image stop, IF `JIT_OUTSIDE_TRACE` is set, print the ring oldest->newest + the bad pc — the exact transition out of the image in <10 lines instead of the megabyte `JIT_TRACE` dump. New pure module-level `ring_ordered(&[u64], ring_i) -> Vec<u64>` (power-of-2 mask, skips unwritten zeros). +1 hermetic `ring_ordered_reports_oldest_to_newest_and_skips_unwritten`.
- **EMPIRICAL (real libroblox.so, llvmpipe, EXIT 124, 0 crash):** with only `JIT_OUTSIDE_TRACE=1` (no JIT_TRACE) the V2Init/V2Start out-of-image stop source is now deterministic: a repeating loop `0x102b9df10 -> 0x106249ecc -> 0x1062514c0` then bad pc `0xf8838b48028948`/`0x40438948028b4800` (= host x86-mcode bytes `48 89 8b`, confirming a HOST code pointer in a guest vtable slot), emitted by `blr x8` at 0x106251eb4 (`ldr x8,[x0]; ldr x8,[x8,#280]; blr x8`) through a HOST-heap object `x0=0x55e...`. **VERDICT:** the residual V2Init/V2Start stop is the SH176/SH103/SH109 singleton-vtable class — a HOST heap/mcode address, NOT a guest address, so NOT a static-seedable slot (pc varies every run: 0x9e/0xcd/0x8b/0xc0e0../0x4043../0x55e7..). It dispatches only after a live registry/object world-build populates the vtable = same migration-gated dependency as the Route-B resolver map (SH193/194/197b). No new headless seed claimed.
- **ROUTE-B POSITIVE:** the governor MODERN startAppWithParams blob-path 0x258c6e4 is REACHED this cycle (region-watch region 0x10258b144..0x10258b400 + 0x10258c6e4 both fire) — closes SH197 §6 lever (a)'s "does the blob-path execute" question with a YES; the residual stop is the non-seedable host-pointer class. SH197 do-init->app-shell->governor->tail continuation re-verified clean.
- **GOVERNOR-TAIL TERMINAL (measured, extends SH197):** watching the full governor region [0x102e9fa84, 0x102ea4000] in the same run, the tail's deepest execution is `0x102ea3084/0x102ea30d0/0x102ea30dc` — a stack-canary `ret` thunk (`ldp x29,x30,[sp,#32]; ret`). Execution NEVER reaches the larger fn at 0x2ea3b14 (`ldr x0,[x0,#688]` — the deeper app-bridge/world-build body). So the manufacture/DMCONT continuation ENDS cleanly at the governor-tail ret; the next real construction fn is unreached headlessly. This is the same live-world-build gate: those deeper bodies only run when a real session populates the governor/app-bridge internals. Extends the SH197 picture (one-level-deeper governor region) with a clean terminal, 0 crash.
- **NEXT (honest):** the V2 out-of-image stop is definitively non-seedable (host ptr, evidence now deterministic & documented) — do NOT chase static seeds for it. Route-B live-DM/class-registry world-build remains the standing structural gate. Any future V2 fix must target the live registry populating vtable slot +0x118 of the 0x55e... host-heap object (via real session), not a seed.
- **CODE (arm64jit jit.rs):** JIT_REGION_WATCH parsed a SINGLE `lo-hi` pair (split_once('-')) — a comma-separated multi-region spec was silently ignored and read as "region not entered", i.e. it FABRICATES a recon/region negative (the footgun behind earlier "governor not reached" reports). Added pure `region_watch_contains(spec,pc)` (comma ranges, [lo,hi), malformed skipped, garbage safe) + once-only unparseable WARN + per-pc dedup. +1 hermetic test.
- **EMPIRICAL at HEAD (real libroblox.so, seeded ladder, ONE run watching all three regions):** region hits across **do-init post-body 0x1023eff4c**, **app-shell ctor 0x102207b50** (entry + 0x102207b88..0x102207c2c → app-data-model register 0x2208354), **governor 0x102e9fa84** → **governor tail 0x102e9fdc8/0x102ea30dc** — the whole do-init → app-shell ctor → governor manufacture/DMCONT continuation executes real relocated engine code headlessly, **0 crash, EXIT 124**, StartLuaAppDM returns real heap addr (not 0x3e8), governor vtable resolves to real `0x102e9fa84` (vt18_is_gov=true). The operator's "hunt the DMCONT continuation inside this JIT" is executing, not parked.
- **RUN-VARIABLE (unchanged wall):** V2Init/V2Start occasionally stop at tiny `pc 0x229/0x188 outside image` (unseeded singleton-vtable host-pointer class, SH176/SH103/109); the StartLuaAppDM→do-init rung completes clean. Live-DM world-build (once-slot 0x400000b = RTApp intern, not a DM) + R1 content remain the standing gate beyond the governor tail.
- **NEXT (honest):** trace what 0x102207b50's app-data-model register (0x2208354) builds now that the continuation is live (self-built DM controller vs the intern), and/or drive the governor MODERN startAppWithParams blob-path past the run-variable outside-image stop.
