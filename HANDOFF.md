...
- **Open residual (low priority):** empirical probe saw the fabricated "Home" jstring build an EMPTY RBX-string at [sp] (w19=0x1); source wires slot 169 to the identity shim but the run shows empty -> two agents disagree (un-hit/stale slot vs valid). Telemetry-only => not chased ahead of the live-DM wall. Live-DM structural wall unchanged (~19 recon angles).
