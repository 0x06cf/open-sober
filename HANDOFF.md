...
## SH95 (Sep 13, 2026, hermes-worker): stop seeding the pb_defaults rwlock-ptr slot 0x106838378 with the map pointer — CLEARED the pthread_rwlock_unlock heap-crash. When the registrar completed (SH94), the do-init tail (`mov w0,#1; ret` at file 0x29b39d4) called pthread_rwlock_unlock (guest 0x102a1ce5c) with x0=[0x106838378]; SH88's seed list [0x106838368,378,380] had seeded 0x378 with the MAP ptr, but 0x378 (offset 888) is a pthread_rwlock POINTER slot — unlock on a garbage lock word does an indirect elision jump -> SIGSEGV fault==rip==heap. Only 0x106838368 + 0x106838380 are the registry-MAP slots (0x380 = INSERT map at +896). Fix: seed only those two; leave 0x378 as the .bss-zeroed (valid unlocked) rwlock. VERIFIED: 0x102a1ce5c crash GONE; registrar completes; ladder advances to a deeper NATIVE fault (0x1029f4034 INSERT-tail, fault==rip==host-heap, rbx_matches_gueststate=false — SH55/64 concurrent-thread/block-cache class now that gameGlobalInit runs much deeper). Also SH95b: registered the SH88 substitute map's leaked bucket array in jit TRUSTED_BUCKETS so the SH91 phantom-slot scrub covers it (it's the map SH92 routes all INSERT into). Workspace 516/0. Product unregressed (124, 0 crash, persist byte-exact). Doc frontier-sh95-rwlock-slot.md. NEXT: resolve the 0x1029f4034 native heap-crash (SH55/64 class — is it a new guest fn-ptr/vtable slot holding host garbage -> seed to in-image fn/host-call, OR the concurrent block-cache desync -> serialize/gate the render thread during do-init) -> then gameGlobalInit RETURNS -> rung 2 nativeUpdateAdapterInit (0x10221c3ec) -> type-4 [0x106829ea8] -> NativeHelper.
