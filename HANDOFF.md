...
## SH92 (Sep 13, 2026, hermes-worker): substituted NON-FAMILY map/this at the INSERT entry — CLEARED the OTel registrar loop gate. Recon deleg_03083bd6: the registrar loop (file 0x29b3814) reads its INSERT map from [0x106838380], which held the .data descriptor-table base 0x1067da308 (in-image, so SH88's v<0x100000000 predicate passed it; a NON-coherent object). INSERT ran with a garbage map -> its +0x30 stack-spill slot became a code-fetch target (fault==rip==host stack at 0x1029b3828) — a new SH86b foreign-object-as-map instance. Fix (jit.rs): extend substitution to INSERT entry 0x1029f3e70 — overwrite x0/x19 with the seeded substitute iff non-family (m<0x100000000 OR [m+0x10] not in {0x1029b4a84 span, 0x102a25dec string}); real family maps + the substitute itself (has +0x10==SPAN_HASH) never re-substituted (idempotent). VERIFIED: substitution fires ~1.25M AND 0x1029b3828 SIGSEGV GONE on every run — the ladder runs the FULL OTel/pb_defaults registration with NO crash; nativeInitializeNativeFlags returns cleanly; nativeGameGlobalInit runs to timeout (exit 124, stable) with render thread producing frames concurrently (frame-fn + swap Ok(0x1)). Workspace 516/0 (+1 regression). Product unregressed (124, 0 crash, persist byte-exact). (4 same-run test 'failures' were ENOSPC from a full /tmp; pass after cleanup.) Doc frontier-sh92-registrar-nonmap-substitute.md. NEXT: the standing gameGlobalInit PARK — feed the next init step so it RETURNS (rung 2 nativeUpdateAdapterInit 0x10221c3ec -> rungs 2-6 install type-4 vector [0x106829ea8]) -> then wire NativeHelper callbacks.
