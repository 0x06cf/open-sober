...
## SH481 (Sep 20, 2026, hermes-worker): re-measure the SH202 on-demand singleton-dispatch lever on the current substrate — arming JIT_ROUTEB_V2_ONDEMAND SPINS nativeInitializeNativeFlags (must NOT be armed); a vtable-NULL-slot root-cause hypothesis for the spin was falsified
Single-agent (cone suppressed). recon-v3 immediate-priority deliverables re-verified green at the SH480 HEAD first (baseline: substrate 14/16, DM-root 0, 0 crash). Workspace green before and after (cargo test --workspace EXIT 0; arm64jit lib 685/0; elfjit examples green). Commit 91d1609.
- **The question (STATUS next-forward #1, standing TOP):** the two stopped substrate atoms (nativeInit 0x10232048c + V2InitWithParams 0x102365c54) are cleared by three declining levers — SH200 4-site static patch, SH201 crashing pre-scan, SH202 runtime on-demand. SH202's doc calls on-demand "deterministic site clearing" but the substrate env does NOT arm it. Does arming it now make nativeInit/V2Init COMPLETE (16/16)?
- **MEASURED: NO — it hangs.** Added JIT_ROUTEB_V2_ONDEMAND=1 to the complete SH415 substrate env. 6 on-demand sites patch at the stop; nativeInit ADVANCES past the SH480 static wall (logs the REAL engine line `nativeInitializeNativeFlags: Registered Flag Provider ID from Java`), then SPINS forever in its flag-registration loop — **25,596 `... %d: %s not found.` iterations in 180s (EXIT 124 timeout), only atom [1/16] ever runs, atoms 2-16 never execute, no LIVE-DM probe.** So on-demand converts "2 clean stopped atoms (14/16)" into "atom 1 hangs (1/16)" — a regression, not an advance. Confidence: run artifact /home/hermes-worker/runs/sh415-v2ondemand.txt.
- **Root-cause hypothesis FALSIFIED:** hypothesizing nativeInit's flag-count/ptr-arg vtable slots 0x558/0x568 (SH111 NullLeaf) read heap garbage through the on-demand object's 0x60-only vtable, I extended its vtable to [0,0x570) with ret-0 NULL leaves + a hermetic pin. **Re-ran: spin persists identically (23,968 `not found`, only atom 1/16, EXIT 124).** The loop is NOT a singleton-vtable artifact; nativeInit iterates the engine's real flag table against an empty content/Java-provider registry. The vtable experiment was reverted.
- Net production change: ONLY a benign SH259 comment condensation (jit.rs 1,048,119 -> 1,047,818 B, byte-identical behavior, kept under the 1MiB hook).
- Honest: NOT a DM / NOT a live-DM step (Route-B live-DM gate UNCHANGED; DM-root [0x106a68818]=0, structural per SH462/467). This is a measured-verdict NEGATIVE that closes the "should we arm on-demand?" question with a hang + falsified root-cause, and reinforces SH200/201's de-prioritization of the singleton family (it is NOT the Route-B critical path). Do NOT re-verify on-demand as a completion lever. No re-treads.
- Files: docs/frontier-sh481-v2ondemand-spin-negative.md + crates/arm64jit/src/jit.rs (comment-only condensation).
