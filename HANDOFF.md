...
## SH171 (Sep 15, 2026, hermes-worker): SendAppEventOnAppReady discriminator DECISIVE recon — the w19-event pin is telemetry-only, NOT a Route-B gate; "Home" resolves to w19=5 (not 4, which is "Game"); whole continuation is FLog + local 0x58 event struct (no DM/governor/ScriptContext). No production change; probe confirmed event string at [sp] is empty -> fallback w19=1. Live-DM structural wall unchanged. Doc docs/frontier-sh171-sendapp-discriminator-telemetry.md. Canonical ladder re-verified on this tree.
- **w19 pin ANSWERED (read-only disasm deleg_76cbd070):** discriminator 0x2bb46b8 maps event first-4-bytes: "Game"(0x656d6147)->w19=4, "Chat"(0x74616843)->w19=1, "More"(0x65726f4d)->w19=5; **"Home"(0x656d6f48) is len-4 but matches NONE -> w19=5**, contradicting the operator's "Home in x5 -> w19=4" premise. Empirical probe: the fabricated "Home" jstring builds an EMPTY RBX-string at [sp] (SSO len 0) -> discriminator takes the fallback w19=0x1. Continuation (0x2bb47d0..0x2bb49e4): version-gate on 0x683d350/0x683d358 (NOT 0x683d348) -> FLog "eventFeature = {}" -> build local 0x58 event struct (w19 at +0x50) -> dispatch 0x2baeeec -> free 4 strings. Purely a telemetry/event-stat object; w19=4 vs 1 vs 5 changes ONLY the logged int. **Not a session-advance lever.** Live-DM wall unchanged (~19 recon angles).
