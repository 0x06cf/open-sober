...
## SH164 (Sep 15, 2026, hermes-worker): 3-agent cone maps the live-DM wiring — the next implementable artifact is a harness DYNAMIC TRACE of the DM ctor, not a seed. Workspace green (538/0). Commit <SH164COMMIT>. Doc docs/frontier-sh164-recon-live-dm-wiring.md.
The 3 read-only agents (deleg_632de38c task-0/1/2) converged on the DM-creator path the SH163 tail cannot reach:
1. First-login DM manager = `NativeDataModelManager` (make_shared), lifecycle getFlagsFromEngine_ -> initEngine_ -> continueAfterFlagsLoaded_(string) -> initializeLuaApp_ -> startLuaApp_. First hard gate is a NETWORK feature-flag fetch (comms gate, bypassable via identity-shim/flags layer, NOT a .bss seed).
2. `UgcExperienceController` / `LuaAppExperienceController` derive BaseExperienceController; createDataModelForTeleport (guest 0x102e1dc2c lambdas, fed by registry 0x1063985f0) is a teleport/downstream branch. NOTE unresolved tension: task-0 attributes primary first-login DM creation to NativeDataModelManager, task-1 points at createDataModelForTeleport — RECONCILE at implementation by disassembling both callers.
3. DataModelServices::setDataModelToCurrent registry FULLY MAPPED: contiguous std::function<void(DataModel*)> objects in .data.rel.ro guest 0x1063915a0..0x106392600; setDataModelToCurrent entry 0x1063918f8, code ptr 0x102dbcc10, dispatch vt 0x635eec0. All tails dispatch on current-DM != NULL; NULL -> benign stubs 0x101db2cf0/0x1021e96f8 (no faults). [0x106a68818] confirmed app-bridge obj+0x20 (DM-root), separate from DataModelServices state, still not the holder.
4. GuiObject->scene-node append site still not statically derivable (walker 0x105b2ed48, 0 direct xrefs). Append site needs a runtime trace.
NEXT implementable artifact (NOT a seed): `sizeof(RBX::DataModel)` is not static (no __shared_ptr_emplace<DataModel> typeinfo; ctor/vtable targets hide in packed ANDROID_RELA reloc stream) — a harness DYNAMIC TRACE driving the DM creation path is required to pin sizeof + the Instance->WorldRoot->DataModel MI chain. Do NOT re-chase: [0x10683cf38] no-op, synthetic CoreScript dead, DataModelPatch feeding (downstream), do-init ctor callees. All Route-B scope.
