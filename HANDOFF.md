...
## SH225 (Sep 16, 2026, hermes-worker): GlobalInit do-init DM-construction dispatch fork RESOLVED to a pinned contract (single-agent Route-B re-attack) — turns SH186's "judged same-difficulty, never built" fork into a concrete byte-anchored target so the next Route-B drive (or proof-of-dead-end) starts from disasm, not an ROI judgment. Commit <SH225COMMIT>. Doc docs/frontier-sh225-doinit-dm-dispatch-fork-pinned.md. +1 hermetic (sh225, elfjit example 73/0). Workspace green.
