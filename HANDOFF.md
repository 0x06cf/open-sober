...
## SH103-SH105 (Sep 13, 2026, hermes-worker): cleared 2 more gates then root-caused the next. SH102+SH103 (commit 4b094e9): forced the ClientRunInfo base-url log gate (`ldr w8,[x27,#14]; cmp #1; b.lt` at 0x102dade08) to always skip (it derefs a never-inited bss singleton *(0x106ED7A28) -> ldrb [x0]=0 fault at 0x102dade34) AND patched the keyed-registry static read `ldr x24,[x20]` at 0x101db5ffc -> `mov x24,#0` (intended NULL; x20 had a host pointer from a PLT strlen host-call). Both null-guarded diagnostic/static reads. Ladder advanced into the scheduler/cond path, then STACK-SMASHES (`__stack_chk_fail`, guestpc=0x0). SH104 (reverted): no-op shims for pthread_cond_broadcast/signal/destroy+mutex_destroy did NOT stop it (ran 9x, smash persisted) — cond_broadcast was not the writer. SH105 (probe, removed): the failing frame's stored canary [x29-16]='0x559097645e50' = a HOST-ASLR pointer, guard [x22] intact — so a HOST pointer leaks into GUEST MEMORY via a host-call excursion (SH103 class). This is a JIT bridge/hosting bug, not a struct-size shim; forcing the canary is UNSAFE. NEXT: pin the exact host-call/bridge path writing 0x5590... into the guest stack canary slot. Doc frontier-sh104-105-stacksmash-hostleak.md. Workspace 518/0. (Historical SH-chain below.)
