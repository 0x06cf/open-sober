...
## SH96 (Sep 13, 2026, hermes-worker): re-assert the coherent 0x400 map header on EVERY INSERT entry — clears the post-growth index-overflow chain-walk (the 0x1029f3f7c image-code-as-node fault is no longer the crash site). Recon deleg_9e27d070: the crash map IS seeded+trusted (so SH91/95b's trust layer didn't miss it); the ENGINE'S OWN GROWTH (file 0x29f3ec0, `str x0,[x19]`@0x29f3ee4) re-writes the map's numeric header (mask +0x40/div +0x44/cap +0x3c/load +0x48) AFTER the once-per-map SH90 seed, so a later insert's `idx*8` overflowed the owned 0x2000 array into image memory (crash x22=0x1029b37f4 code page) — the SLOT ADDRESS overflows, not a phantom slot value. Fix (jit.rs INSERT entry): for a trusted family map, RE-ASSERT +0x3c=0x400/+0x40=0/+0x44=0x400/+0x48=0x100/+0x60=0/+0x58=0 so idx*8 stays in the owned 1024-slot array regardless of growth re-writes + keep the SH91 in-image slot-value scrub; never repoint +0x00 (SH84/86b). VERIFIED: old 0x1029f3f7c guest chain-walk fault gone; runs now fail at a DIFFERENT host-side class (guestpc=0x0, fault==0xffffff80ffffffc8 or fault==rip==heap, rbx_matches_gueststate=false, run-variable 134/139 — the SH55/64 concurrent-thread/block-cache class now that gameGlobalInit runs much deeper). Workspace 516/0. Product unregressed (124, persist byte-exact, 0 crash). Doc frontier-sh96-header-reassert.md. NEXT: resolve the host-side fault class (is it a guest fn-ptr/vtable slot with host garbage -> seed to in-image fn/host-call, OR the concurrent block-cache desync -> serialize/gate the render thread during do-init) -> gameGlobalInit RETURNS -> rung 2 nativeUpdateAdapterInit (0x10221c3ec) -> type-4 [0x106829ea8] -> NativeHelper.
