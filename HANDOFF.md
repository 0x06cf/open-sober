...
## SH91 (Sep 13, 2026, hermes-worker): scrubbed phantom image-range bucket slots at the insert entry — CLEARED the 0x1029f3f7c node-pointer fault. Recon deleg_f77df07a: crash-map header/array were valid + never reallocated (not growth overflow); a single bucket slot held the IMAGE address 0x1029b37ec (a .text thunk written as a phantom head link after ~thousands of inserts into the never-grown 1024-slot map). The chain-walk `ldr x23,[x22]` (file 0x29f3fb4) picked it up and `ldr x8,[x23,#16]` deref'd image+16 (SIGSEGV 0x1029f3f7c). Node layout (24B): +0x00 key(value), +0x08 next, +0x10 hash. Fix (jit.rs): at INSERT entry scrub any slot value in guest image range [base,base+len) -> NULL (valid node ptrs/host metadata are NEVER in-image); only touches arrays WE own (shared TRUSTED_BUCKETS set, avoiding a Rust panic from a foreign map's invalid +0x00); no +0x00 repoint (SH84/86b). VERIFIED: 0x1029f3f7c SIGSEGV GONE; ladder now faults DEEPER at guestpc **0x1029b3828** — the OTel/pb_defaults registration CALLER loop (iterates .data table at 0x1067da308 inserting into registry map [0x106838380], `ldr x8,[x19,#16]!; cbnz`) with fault==rip==host-heap (blr-into-heap, SH86 class). Workspace 515/0. Product unregressed (124, 0 crash, persist byte-exact). Doc frontier-sh91-phantom-bucket-scrub.md. NEXT: clear the 0x1029b3828 registration-loop gate (trace which blr jumps into the heap — extend SH86 operator-new seed or SH81 vtable/host-call seeding) -> then nativeGameGlobalInit returning -> rung 2 -> type-4 vector [0x106829ea8].
