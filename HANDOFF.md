...
## SH238 (Sep 17, 2026, hermes-worker): CROSS StartLuaAppDM's receiveCall dispatch-select to the EC INVOKE slot (SH235-237's "next drive") + MEASURE — the select lever is a MEASURED DEAD-END (forcing invoke does not advance; the marshaler 0x1023f075c -> EC world stays 0 hits), and SH236's "__clone soft-return" inference is CORRECTED ([sp+32] is natively a REAL pointer 0x5624f64f6a50, so the dispatch natively takes INVOKE, not __clone). +1 hermetic (sh238, arm64jit lib 390/0). Default-inert lever JIT_ROUTEB_SLADM_INVOKE. Doc docs/frontier-sh238-sladm-invoke-cross.md, repro runs/capture_sh238_sladm_invoke.sh. Workspace green.
