...
NEXT (in priority): the combined-ladder+frame gap is the drain-not-resumed structural issue — candidates are driving the drain pop-loop 0x102856e40 as a top-level jit_run on the renderinit thread after LADDER_DONE (risky, SH44 re-eviction class), or accepting ladder-only + the standalone self-driven-frame plane as the reproducible product artifact (both proven clean). Session-advance (real login/home) remains behind the Lua app-shell wall. **PREREQUISITE CONFIRMED (runs/sh127-fwdump.txt + deleg_b193ac24):** the drain is a callee of the engine's idle-main-loop on the MAIN thread; in the combined path StartApp RETURNS (not parks), so no guest thread drives the drain — BUT its deque-fwd globals stay coherent after return (0x1068262e8=0x10620db24, 300=0x102176bfc, 308=0x1022199e0) and the type-4 vector stays seeded (0x7f00000001d8), so the LOW-RISK re-drive (bounded top-level jit_run of the idle-main-loop post-ladder, NO force-pop/block-eviction) is VIABLE. Next prerequisite: read-only disasm of the idle-loop ENTRY pc (lr 0x10284d134 is the in-loop futex; 0x102856e40 is the drain body).
