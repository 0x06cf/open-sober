...
## SH101 (Sep 13, 2026, hermes-worker): CLEARED the globalinit do-init walker dispatch-2 gate. SH100 (2f12f06) broke the SH55/64 run-variable SIGSEGV class by gating `clear_block_cache()` behind a process-global top-level-run counter (a concurrent worker/render jit_run no longer evicts blocks mid-ladder). SH101 (d6da355) fixed the walker's dispatch-1: it reads `[vtab+0x10]`, and its RETURN in x0 becomes the guest memzero-loop base — the identity leaf returned obj, erasing obj[0] so dispatch-2 faulted [0x18]. Installed a scratch-returning host leaf at vtab+0x10. RESULT: dispatch-2 CLEARED; the ladder now advances deeper into nativeGameGlobalInit to a NEW deterministic gate guestpc 0x102dade34 (`ldrb w8,[x0]` fault=0x0, x0=0 after bl 0x102b511e0 then bl 0x10221364c). Workspace 518/0. Next: decode the 0x102dade00 region (a string/init helper) -> gameGlobalInit RETURNS -> rung 2 nativeUpdateAdapterInit (0x10221c3ec) -> type-4 [0x106829ea8] -> NativeHelper. (Historical SH-chain below.)
