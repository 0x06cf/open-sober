...
## SH88 (Sep 13, 2026, hermes-worker): substituted the OTel/pb_defaults registry map for sub-image map/this tags — clearing the OTel FIND dispatch gate that survived SH87. Recon deleg_ba208bc6: after the SH87 blr x8->blr x1 patch, the FIND op (file 0x29f424c) was handed a static `.data.rel.ro` protobuf FIELD-TAG constant (0x1800064, from the 16-byte-strided descriptor table at file 0x62f5110) as the map/this arg because the upstream registry map (BSS 0x106838368/378/380) is never constructed under the JIT. 0x1800064 < 0x100000000 is never an image/pointer -> [0x1800064+16] unmapped -> SIGSEGV fault=0x1800074. Fix (elfjit routeb_seed_pb_registry_map + jit routeb_substitute_map): seed a coherent empty span-hash map (SH84 shape: +0x10=0x1029b4a84 real span hash, zeroed bucket array, coherent divisors) into the 3 BSS registry slots, register it, and at FIND entries (0x1029f424c/0x1029f4284) overwrite any non-zero sub-image x0/x19 candidate with the seeded pointer. Verified: substitution fires 492x; FIND gate GONE; run faults one gate DEEPER at the INSERT op (0x1029f3f7c blr x8 through a REHASHED map's fresh +0x18 garbage 0x9401a599f941be80, x0=0x40000d5) — the recon's predicted rehash/erase next gate. Workspace 515/0 (new regression routeb_hashfix_substitutes_subimage_map_candidate). Product unregressed (124, 0 crash, persist byte-exact). Doc frontier-sh88-pb-registry-substitute.md. NEXT: force blr x8->blr x1 at the INSERT dispatch (file 0x29f3f78, same family as SH87) or extend +0x18-non-image-zeroing to the rehash/creation writers -> then nativeGameGlobalInit returning -> rung 2 -> type-4 vector [0x106829ea8].
