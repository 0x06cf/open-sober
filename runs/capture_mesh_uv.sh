#!/bin/bash
# SH143: real Roblox mesh (smooth_sphere.mesh) + real APK texture (studs.dds R8)
# rendered with REAL per-vertex UVs and a REAL camera/MVP through the engine's own
# GLES path. Over SH142's screen-space (planar) sampling, this is the ranked
# render-plane ascent: per-object UV wrap + a perspective MVP uniform transform.
set -u
cd "$(dirname "$0")/.."
...
for i in $(seq 1 150); do
  if grep -q "real MVP uploaded" "$LOG" 2>/dev/null && grep -q "silhouette 5x5" "$LOG" 2>/dev/null; then break; fi
...
wait $PID
echo "EXIT=$?"
echo "=== real mesh + real texture + real MVP parsed? ==="
grep -E "renderframe-mesh\] parsed real|renderframe-mesh-tex\] parsed real DDS|real MVP uploaded" "$LOG"
...
echo "=== pixel histogram (UV-wrapped studs texture over the real sphere) ==="
if [ -s "$RAW" ]; then
