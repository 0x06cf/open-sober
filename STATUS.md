...
## SH111 (Sep 14, 2026, hermes-worker): NativeHelper gameActivity_* callbacks wired via name-dispatching CallVoidMethod; singleton-vtable widening empirically dead-ended (0x60 baseline locked). Workspace 25 suites green; arm64jit lib 341/0, elfjit examples 23/0. Commit 2e4b7f2. Doc frontier-sh111-vtable-nativehelper.md.
- route-B step 2 (first half): CALL_VOID_METHOD was the shared no-op `ok` stub, so the engine's five NativeHelper `gameActivity_*` callbacks silently no-op'd and StartLuaAppDM never advanced its session. New jni_call_void_method dispatches on the method name + records a per-callback milestone atomic (onFlagsLoaded/onEngineInitialized/onAppReady/onGameLoaded -> MH_* atoms; onDidLogInReceived = VOID String arg per disasm, not a jboolean): registered at functions[CALL_VOID_METHOD]=61 AND CALL_STATIC_VOID_METHOD=141. Auto-regression nativehelper_game_activity_callbacks_dispatch_void_method.
- vtable: tried the 0x60->0x580 differential widening a SECOND way (object-leaf default) — STILL regresses nativeInitializeNativeFlags (exit 134). The shared vtable cannot serve nativeInit (clean only via benign read-past soft-return) AND V2Start (needs wide coverage). Reverted to 0x60, regression-sh111 locks the narrow baseline. Correct fix for the three soft-returns = SCOPED patch at the dispatch blr sites (0x62517c4/0x6251aa8/0x6260948), NOT vtable growth.
- Verified the combined-recipe exit-139 `renderinit (bad-ptr)` is PRE-EXISTING (identical on pristine HEAD) — not a regression. No-render clean ladder re-verified: EXIT 0, 0 crash, StartLuaAppDM returned Ok, nativeGameGlobalInit/nativeUpdateAdapterInit/setTaskSchedulerBM/V2Init/StartLuaAppDM/V2Start/V1 all rung.
- NEXT: (a) poll the MH_* milestone atoms to actually advance the session (route-B step 2 tail) + drive the ladder far enough that the callbacks fire; (b) resolve the renderinit `(bad-ptr)` runtime_error so the combined ladder reaches StartLuaAppDM Ok; (c) scoped call-site patch for the three soft-return sites.
...
## SH73 (Sep 13, 2026): the engine's REAL geometry emitter now renders a REAL Roblox LOGIN/auth surface headlessly — the real `reversevignette` dark backdrop + the white RO-BLOX wordmark (`logo_white_1x.png`, 476x88), decoded OFFLINE from the real APK Auth artwork (`ExtraContent/textures/ui/LuaApp/graphic/Auth/`), sampled from a shared vertical atlas in ONE top-level jit_run, GL_BLEND over the dark backdrop. New env RENDEREMITTER_LOGIN=1 (+ MULTI=1 + LAYOUT=home). Real libroblox.so exit 124: 2x `engine emitter Ok(count=18 sprites=2 swap=Ok(1))` + 2x present-walker Ok(ret=0x1), **4/4 probes present=true** — wordmark probe (560,485) **Byte-EXACT** (255,255,255,255 diff=[0,0,0,0]); vignette (641,359) rgba(48,50,52) over dark. Captured runs/sh73-emitter-login.png = recognizable Roblox splash (RO-BLOX wordmark with the tilted-square "O" over a center-glow dark vignette). 0 crash. Code: real_ui_textures honors absolute paths; login_ui_textures(); login placements; spinner probe gated off in login mode. Workspace **509/0** (unchanged). Docs frontier-sh73-emitter-login.md + frontier-sh73-next-artifact.md, repro runs/capture_emitter_login.sh. Honest scope: harness-authored geometry + real artwork through the engine's own emitter — not the engine self-constructing a GuiObject/session; standing wall unchanged.
