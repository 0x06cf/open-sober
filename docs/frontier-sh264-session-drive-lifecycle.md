...
4. [`--v2boot-session-set`] `nativeInitClientSettings` (guest `0x1022265fc`) — the client-settings
   receive entry that feeds `initEngine_`'s "Engine settings is null" hard-assert; its string args
   resolve via `0x21e1fec` (the jstring→RBX-string helper whose JNIEnv GetStringUTFChars slot is
   the identity shim in this harness, SH186 — fabricated empty jstrings DO resolve), gated on the
   version word `[0x10683cff8]`
5. [`--v2boot-session-resumed`, ambulatory-only] `nativeOnResumed` — see "measured" below
...
- **`nativeInitClientSettings`** (`--v2boot-session-set`) **returns `Ok(0x1)` cleanly** — the
  client-settings receive entry that feeds `initEngine_`'s "Engine settings is null" now executes
  headlessly (first time), resolving fabricated empty CLEAR-XML jstrings via the SH186 identity
  shim. It is a pure consumer (no milestone; the four post-lifecycle probes still read false / 0),
  but it proves the "feed real client-settings" lever is driver-able and non-crashing, and the
  app-start ladder still runs to the same parked terminal (no regression).
...
The SEP-17 session-drive workstream is now **wired + measured-latent-but-correct**: four
non-aborting lifecycle natives run as real engine session-init entries (initAppShellReporter +
setActive both complete Ok(0x0); nativeInitClientSettings Ok(0x1)) and are regression-safe. It
does **NOT** manufacture a DataModel; Route-B live-DM structural gate UNCHANGED (the upstream
session ctor still doesn't run far enough to build a DM world). The remaining named levers — the
messageBus **experience-launch** + dataModel-bindings receive entries — are still un-driven; they
are the honest next candidates on this line.
