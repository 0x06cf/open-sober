...
## SH186c — MANUFACTURED-DM→live-binder cb closure + PERSISTENCE migration-closure (2-agent follow-on, authentic)
### (1) Manufactured-DM through the messageBus 'experience-launch request' cb = EXHAUSTED, benign dead-end (deleg task-0)
Re-located cb body = fn 0x2bd744c (mid-function 0x102bd76e8 is just an ADRP+ADD of a log string). On a non-null DM it: `x0=[x0+16]`→DM; cbz→bail; `bl 0x2b4ea48` (AddRef on [DM+8]); `x20=[x0+8]` (app, may be null→skip); **DM consumed ONLY via `ldr x8,[DM_vt]; blr [x8+0x10]` (vtable slot 2) then Release.** Never calls getDataModel, never builds AppShell-reporter/SceneGraph nodes, never touches R+0x180/0x188.
DECODED slot-2 per row (full APS2 reloc decode, 568,272 relocs, stream-exhausted exact):
- vt 0x67162f0 (SH value): [vt+0x10] = 0x229c2a4 = `mov x0,xzr; ret` (null-return, zero derefs) → cb runs AddRef→blr-null→Release = **benign dead-end, no fault, no GuiObject**.
- vt 0x67163a8: [vt+0x10] = 0x240a8b8→0x24090a8 = real StartLuaAppDM consumer; derefs [x0]vptr, [vt+8], then deep DM members (0x668/0x758/0x760/0x860/0x664+) → **SH182 PATH-B deep-NULL fault** on the shallow ctor-only DM.
- vt 0x6716400: [vt+0x10] = 0x57d07f8 = destructor path → faults on shallow object.
**CORRECTION (record it): the three genuine DM vptr values are NOT interchangeable vtable rows** (slot-2 differs per row: null-stub / real-consumer / destructor). The SH181/182 manufacture lever used vt=0x67162f0 (the null-stub row) — which is why feeding it through the cb is inert. Even the consumer row dies on deep NULL members before any GuiObject instantiation. Combined w/ SH186b (zero materialization), Route-B self-constructed scene nodes have NO headless mechanism in this binary. Manufactured-DM line FULLY EXHAUSTED; do NOT re-tread.
### (2) Persistence / session-restore (objective b) = STRICTLY MIGRATION (deleg task-1)
Auth/cookies have ZERO native disk store: no literal for session.db/shared_prefs/cookies.db/ObjectStore; token flow is 100% JNI/env-class (Java CookieManager) — nativeSetMultipleCookies (0x2202ff8) + nativeGetCookiesInNetscapeFormat (0x21ff5a8) are JNI entries, read the JavaVM global in prologue; write-side jar [0x106ed7a20] serialized only by Java. The ONLY real native disk store = rbx-storage.db (content/asset KVS, not auth). Headless ladder touches ZERO guest /data or /cache files (no [fsmap] remap lines; run_persist_roundtrip is a harness backend self-proof, not an engine path). Staging SH168's db or a jar value produces NO engine read-back. => Persistence is behind the live-DM/JVM migration wall; SH168 + SH177 remain correct-but-inert migration readiness. No further persistence-seed investment.